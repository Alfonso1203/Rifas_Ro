...
@st.cache_data(ttl=2) # Actualización rápida
def cargar_datos(url):
    # Saltamos 2 filas para que la fila 3 sea el encabezado (donde están tus datos)
...
try:
    ...
    for _, row in df_full.iterrows():
        try:
            ...
            if celda_nums and celda_nums.lower() != 'nan':
                # Separamos por comas para números como "2,4,10"
    ...
    st.markdown("""
        <div style="text-align: center; font-size: 0.9rem; border: 1px solid #ddd; padding: 10px; border-radius: 10px;">
            ...
            <i style="color: gray;">Actualización automática cada pocos segundos ⌛</i>
        </div>
    """, unsafe_allow_html=True)

    st.write("---")

    # --- 5. DATOS DE PAGO Y WHATSAPP (LETRAS BLANCAS) ---
    ...
    with col2:
        ...
        # Botón con letras blancas (color:white)
    ...
    st.error(f"Conectando con la base de datos... Por favor espera. Error: {e}")
